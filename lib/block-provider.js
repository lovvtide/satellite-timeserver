...
export default class BlockProvider {
	...
	getMajorityConsensus(results, keyExtractor) {
		...
		if (validResults.length > 0) {
			...
			if (count > validResults.length / 2) {
				return validResults.find((result) => String(keyExtractor(result)) === majorityKey);
			}
		}
	...
	async getBlock(blockId) {
		...
		const requests = this.dataSources.map((source) =>
			this.timeoutRequest(source.getBlock(blockId), dataSourceTimeout).catch((error) => {
				console.warn(`${source.label || 'unknown data source'} failed to fetch block`, error.message);
				return null;
			})
		);
